...
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17892
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 17890
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 27892
%{_sbindir}/semanage port -a -t bitcoin_port_t -p tcp 27890
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 17892
	%{_sbindir}/semanage port -d -p tcp 17890
	%{_sbindir}/semanage port -d -p tcp 27892
	%{_sbindir}/semanage port -d -p tcp 27890
	for selinuxvariant in %{selinux_variants}; do
