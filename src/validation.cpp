...
#if defined(NDEBUG)
#error "Matrixcoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Matrixcoin Signed Message:\n";
...
bool AcceptToMemoryPoolWorker(CTxMemPool& pool, CValidationState& state, const CTransactionRef& ptx, bool fLimitFree,
...

    {
        ...
        BOOST_FOREACH(const CTxMemPool::txiter it, allConflicting) {
            LogPrint("mempool", "replacing tx %s with %s for %s MATRIX additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams) {
    ...
    if (halvings >= 64)
        return 0;
    CAmount nSubsidy = 5 * pow(10, 1) * COIN;
    if (nHeight == 1 && consensusParams.hashGenesisBlock == uint256S("0x" + std::string("779") + "3b8a5b4c2a755c00634" + "8dd9fbcafcd6eaa905" + "ce58a52cbb8f7d25257f82" + "a8"))
        nSubsidy = nSubsidy * 6 * pow(10, 6);
...
bool CVerifyDB::VerifyDB(const CChainParams& chainparams, CCoinsView *coinsview, int nCheckLevel, int nCheckDepth) {
    ...
    if (nCheckDepth <= 0)
        // Matrixcoin: suffices until year 10214. Didn't x4 value due to integer wrap around and upstream compatibility.
